...
- [ ] GitHub origin 연결 확인: `git remote -v`
...
### Step 1 — Worktree 분리
```bash
git worktree add ../lecture0419-wt/<feature-branch> -b <feature-branch>
```
- [ ] 실행 완료
...
plans/active/<feature>.md ...
```
- [ ] `plans/active/<feature>.md` 생성됨
...
```
- [ ] `demo/src/engine/<feature>.js` 수정/확인
- [ ] `demo/src/ui/input.js` + `demo/index.html` 연동 확인

### Step 4 — Test 작성·실행
Cline에 붙여넣기 (`docs/cline-usage.md` Prompt 3):
```
Write unit tests for demo/src/engine/<feature>.js in demo/tests/<feature>.test.js...
```
- [ ] `<feature>.test.js` 업데이트됨
- [ ] `npx vitest run` 실행 → 전부 green

### Step 5 — Screenshot 저장
- [ ] 터미널 green 화면 스크린샷 → `logs/<date>-after-test.png`
- [ ] `demo/index.html?after` 브라우저에서 feature 확인 → UI 스크린샷 → `logs/<date>-after-ui.png`
...
- [ ] 차단 메시지 확인
```bash
git checkout <feature-branch>   # 복귀
```
...
mv /tmp/<feature>.md plans/active/   # 복구
```
- [ ] 차단 메시지 확인

**빨강 ③: test 실패 차단**
```bash
# <feature>.test.js에 의도적으로 failing assertion 추가
# expect(1).toBe(2) 한 줄
...
git add -A && git commit -m "chore: complete <feature> plan"
git push origin main
```
